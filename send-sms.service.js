...
class SendSmsService {
  #login = process.env.SMS_GATEWAY_LOGIN || "test@eskiz.uz";
  #password = process.env.SMS_GATEWAY_PASSWORD || "PASSWORD";
  ...
  async sendSMS(phone, template) {
    ...
    try {
      ...
      const response = await axios.post('http://notify.eskiz.uz/api/message/sms/send', data, {
        headers: {
          ...data.getHeaders(),
      ...
      if (e.response.statusText === 'Unauthorized') {
        ...
        throw new Error(`SMS gateway error when sending sms. Error message: ${e.message} (Unauthorized!)`);
      }
      throw new Error(`SMS gateway error when sending sms. Error message: ${e.message}`);
    }
  }
  ...
  async #getTokenAndSave() {
    try {
      ...
      const response = await axios.post('http://notify.eskiz.uz/api/auth/login', data, {
        headers: {
          ...data.getHeaders()
        }
      });
